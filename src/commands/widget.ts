 ...
 */
function widgetsWriteKey(raw: Record<string, unknown>): 'customWidgets' | 'customCommands' {
  if ('customWidgets' in raw || 'customCommands' in raw) return resolveWidgetsKey(raw);
 ...
 */
function readWidgetsBlock(
  raw: Record<string, unknown>,
): { key: 'customWidgets' | 'customCommands'; enabled: boolean; commands: unknown[] } {
...
async function cmdEnable(name: string): Promise<Result> {
  try {
...
async function cmdDisable(name: string): Promise<Result> {
  try {
    const raw = readConfigRaw();
    const block = readWidgetsBlock(raw);
    const updated: Record<string, unknown> = {
...
async function cmdList(name: string): Promise<Result> {
  const c = makeColors();
  let enabled = false;
...
async function cmdTest(name: string, id: string | undefined): Promise<Result> {
  if (!id) {
    return fail(
      `lumira ${name} test: missing command id.\n\n`
  ...
  if (!cmd) {
    ...
    return fail(
      `lumira ${name} test: command id "${id}" not found.\n`
 ...
 */
export async function runWidgetCommand(argv: string[]): Promise<Result> {
