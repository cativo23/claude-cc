import type { CustomCommandValueTier } from '../types.js';
...
export function parseWidgetValue(text: string): number | null {
 ...
 */
export function matchValueTier(
  tiers: CustomCommandValueTier[],
  value: number,
): CustomCommandValueTier | undefined {
  for (const tier of tiers) {
    if (tier.lt === undefined || value < tier.lt) return tier;
  }
  return undefined;
}
