...
export function renderLine2(ctx, c) {
    ...
    if (display.tokens) {
        ...
        if (parts.length > 0)
            leftParts.push(`${icons.comment} ${parts.join(' ')}`);
    }
    ...
    if (display.health && input.cwd) {
        ...
        if (hints.length > 0) {
            const candidates = hints.map(h => h.severity === 'warn' ? c.yellow(`⚠ ${h.hint}`) : c.dim(`ℹ ${h.hint}`));
