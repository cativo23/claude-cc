...
function buildSegments(ctx, palette, c) {
    ...
    if (display.branch && branchName) {
        ...
        if (showDirty) {
            const badges = [];
            if (git.staged > 0)
                badges.push(`+${git.staged}`);
            if (git.modified > 0)
                badges.push(`!${git.modified}`);
            if (git.untracked > 0)
                badges.push(`?${git.untracked}`);
            if (badges.length)
                label += ' ' + badges.join(' ');
        }
        segments.push({
    ...
    if (display.version && input.version) {
        segments.push({
            text: hyperlink(`https://www.npmjs.com/package/@anthropic-ai/claude-code/v/${encodeURIComponent(input.version)}`, `v${input.version}`),
            bg: palette.versionBg,
            fg: palette.fg,
            priority: 20,
        });
...
export function renderPowerlineLine1(ctx, mode, theme) {
    const palette = theme
...
//# sourceMappingURL=powerline-line1.js.map